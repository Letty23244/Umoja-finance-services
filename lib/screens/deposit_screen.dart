...
class _DepositScreenState extends State<DepositScreen>
  ...
  void initState() {
    super.initState();
    _currentBalance = context.read<AccountProvider>().walletBalance;
    ...
    try {
      ...
      if (response["status"] == "success") {
        final newBalance = double.tryParse(
              (response["new_balance"] ?? 0).toString(),
            ) ?? _currentBalance;

        setState(() => _currentBalance = newBalance);

        if (mounted) {
          context.read<AccountProvider>().applyDeposit(
            newBalance,
            double.tryParse(confirmedAmount) ?? 0,
            'Cash Deposit',
          );
        }

        _showSuccessSheet(confirmedAmount);
        _amountController.clear();
        _descController.clear();
      } else {
        _showSnack(response["message"] ?? "Deposit failed",
            isError: true);
      }
    } catch (e) {
      setState(() => _isLoading = false);
  ...
  void _showSnack(String msg, {bool isError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(msg),
        backgroundColor:
            isError ? Colors.red.shade700 : accentGreen,
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _sectionLabel('Amount to Deposit',
                            Icons.monetization_on_outlined),
                        const SizedBox(height: 10),
                        _buildAmountCard(),
                        const SizedBox(height: 28),
                        _sectionLabel('Deposit Method',
        ...
        const Expanded(
          child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text(
                    'Visit our nearest branch to deposit your funds.',
                    style: TextStyle(
                        color: lightGreen,
                        fontSize: 11,
                        height: 1.5)),
      ...
      child: Stack(children: [
        ...
        Padding(
          padding: const EdgeInsets.only(
              left: 24, right: 24, bottom: 28, top: 70),
          child: Column(
          ...
          children: [
            Column(
                ...
                children: [
                  ...
                  Text(fmt.format(_currentBalance),
  ...
  Widget _buildAmountCard() {
    return Container(
      ...
      child: Column(children: [
        Padding(
          padding: const EdgeInsets.only(
              left: 20, right: 20, top: 18, bottom: 6),
          child: Row(children: [
            Container(
                padding: const EdgeInsets.symmetric(
  ...
  Widget _buildDescriptionCard() {
    return Container(
      ...
      child: TextFormField(
        ...
        maxLines: 3,
        style: const TextStyle(
            fontSize: 14, color: darkBrown, height: 1.5),
        decoration: InputDecoration(
  ...
  Widget _buildInfoBanner() {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
              color: accentGreen.withOpacity(0.25), width: 1.2)),
      child: Row(children: [
        Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(
        ...
        const Expanded(
            child: Column(
              ...
              SizedBox(height: 3),
              Text(
                  'Your deposit is secured and reflected instantly.',
                  style: TextStyle(
                      fontSize: 11,
                      color: primaryBrown,
                      height: 1.5)),
  ...
  void _showSuccessSheet(String amount) {
    final parsedAmt = double.tryParse(amount) ?? 0.0;
    final displayAmount =
        NumberFormat.decimalPattern('en_US').format(parsedAmt);
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
              'UGX $displayAmount via Cash Deposit\nhas been added to your account.',
              textAlign: TextAlign.center,
              style: const TextStyle(
                  fontSize: 13, color: mutedText, height: 1.7)),
          const SizedBox(height: 32),
          Container(
            width: double.infinity,
            height: 54,
